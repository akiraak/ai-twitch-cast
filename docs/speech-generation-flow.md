...
comment_reader.speak_event(event_type, detail, voice=None, style=None, avatar_id="teacher", multi=True)
  ...
  └─ マルチ (multi=True かつ生徒キャラあり):
       ...
       全エントリの TTS を並列生成 → 全 WAV 揃ったら entries 組み立て
       → DB 保存（バッチ送信前にまとめて）
       → 最初のエントリの _post_to_chat を 2秒遅延バックグラウンドで発火
       → speech_pipeline.speak_batch(entries)  ← C# キューに一括送信・順次再生
       → finally で apply_emotion("neutral") + notify_overlay_end
  │
  ▼
シングルのみ: apply_emotion(emotion) → speak(...) → apply_emotion("neutral")
マルチは上記のとおり speak_batch が字幕・口パク・感情切替を C# Push 駆動で発火
```
...
**複数エントリ掛け合いの並列TTS事前生成**: マルチキャラの掛け合い（2〜4エントリ）では、全エントリのTTSを `generate_tts()` + `asyncio.create_task` で並列起動し、各エントリを事前生成 WAV として用意する。再生方式は呼び出し元によって2系統ある:

1. **チェーン再生（バッチ送信）** — `comment_reader.speak_event()` マルチパス、`claude_watcher._play_conversation()`: 全 WAV 揃ったら `speak_batch(entries)` で C# キューに一括投入。下記の「Claude Code実況のチェーン再生」参照
2. **直列再生（per-entry speak）** — `respond_webui()`、`_respond()`: 先頭から順に `await` → `speak(wav_path=事前生成WAV)` で再生。エントリ間の「間」は `asyncio.sleep(0.3)` の固定値
...
従来は各エントリごとに直列でTTS生成→再生していたため、3エントリで 1〜4.5秒の余計な待ちが発生していた。並列化後は直列再生パスで 0.6秒（0.3秒×2）固定、チェーン再生パスではほぼ0秒。
...
- 呼び出し箇所: `claude_watcher._play_conversation()` および `comment_reader.speak_event()` マルチパス（他モードは上記の直列パスを維持）
- 割り込み時: `claude_watcher._play_conversation()` は `comment_reader.queue_size > 0` を監視タスクが検知 → `capture_client.cancel_tts_batch()` で C# キューをクリア。`comment_reader.speak_event()` マルチパスは（Claude Code Hook / コミット報告など短時間発話のため）割り込み監視を持たず、最後まで再生する
...
- **ハング耐性（`plans/tts-batch-playback-hang-fix.md`）**: `PlayTtsLocally` は以下3つの多層防御でチェーン途絶を防ぐ
  1. `WaveChannel32.PadWithZeroes = false`: NAudio デフォルトの無音ゼロパディングを無効化し、WAV 終端で `Read()` が 0 を返して `PlaybackStopped` が自然発火するようにする（これがないと 1 件目で永久に詰まる）
  2. duration ベースのフォールバック: `reader.TotalTime.TotalSeconds + 1.5s` を `Task.Delay` して PlaybackStopped 未発火時に強制的に `DequeueAndPlayNextLocal(finishedCurrent: true)` を呼ぶ（`Interlocked.CompareExchange` で PlaybackStopped と原子化）
  3. `OnTtsAudio`（単発）はバッチ中断時に `_ttsLocalCurrent = null` もクリア。単発がさらに中断される合わせ技で `_ttsLocalCurrent` が残留し、後続バッチが `wasIdle=false` で弾かれるケースを防ぐ
