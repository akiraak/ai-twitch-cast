...
- [x] **対象**: `CLAUDE.md`（新セクション「配信動画のクロップ＆音量ノーマライズ（debug-ss/）」）
- [x] **内容**:
  - クロップ座標: `x=1, y=38, w=1280, h=720`（broadcast canvas を切り出す位置。タイトルバー・右サイドバー・下部ステータスバーを除外）
  - 音量目標: -16 LUFS / TP=-1.5 dBTP / LRA=11
  - 2 パス処理を必須化（シングルパス `loudnorm` は冒頭の音量が徐々に上がる挙動になるため）
  - 1 パス目（測定）と 2 パス目（クロップ＋ノーマライズ＋エンコード）の ffmpeg コマンドを記載
  - 出力ファイル名規則: `<入力>_cropped.mp4`
...
- [x] **`prompts/lesson_improve.md` 反映**: ルール末尾に「セクション冒頭に『画面を見てね』『読み上げるよ』『画面に〜を並べたよ』のようなメタ前置きを入れない」を追記。改善ループで前置きが再注入されるのを防止
- [x] **既存セクション**: 戦略4（何もしない）採用。Lesson #1 は試聴段階なので、配信フィードバック次第で improver による部分修正を判断（一括スクリプト処理はTTS再生成コストと誤検出リスクが見合わないため見送り）
- [x] **要件Aの維持**: `display_text` 全文読み上げの強調を残しているので、「省略・要約」に LLM が走るリスクは低い。新規生成サンプルでキーワード（画面を見/画面に/読み上げ等）の不在と全文読み上げの両方を確認する方針
- [x] **プラン**: [plans/lesson-preamble-removal.md](plans/lesson-preamble-removal.md) — ステータス「完了」
...
- [x] `[AVSync]` ログ + `ffprobe` で計測:
  - audio queue depth: 99% が 0〜1、最大 10（cap 到達） → 蓄積遅延は解消
  - fps 30 / ドロップ率 1.9% / パイプ slow 3 回 → B1 副作用は完全解消（B1 副作用前と同等）
  - MP4 duration 差: +0.85s（B1 前 +1.62s → 改善）
  - 音声ドロップ 2055（≒20s分、3.5%）
- [x] VLC 目視（ユーザー報告）: 「ぶつぶつ音切れ」「0.5 秒の遅延ばらつき」が新たに発生 → cap=10 が低すぎてジッタを吸収できず古いチャンク捨て頻発が原因と特定
- [x] `plans/recording-av-sync-fix.md` を更新: B2 計測結果の数値表 + 新症状の原因分析 + C+A 案（cap=30 / offset=-0.3）を追記、ステータスを「C+A 案へ進む」に変更
- [x] `TODO.md` を更新: 次の作業を C+A 実装に切り替え
