...
Twitch API連携、画像生成・収集、テキスト生成、音声合成など、配信に必要な要素をすべてプログラムで生成・制御する。Electronアプリ（Windows）でbroadcast.htmlをオフスクリーンレンダリングし、FFmpegでTwitchに直接配信する。

## 主な機能

- **Electron配信パイプライン** - Windows側Electronアプリでbroadcast.htmlをレンダリング→FFmpegでTwitch直接配信（OBS/xvfb不要）
- **AIアバター** - VRMアバターが感情連動で表情変化、TTS音声で発話
- **Twitchコメント応答** - チャットをAIが読み取り、キャラクター設定に基づいて応答・読み上げ
- **配信オーバーレイ** - TODO表示・字幕・情報パネルを配信画面に合成
- **ウィンドウキャプチャ** - Windows側のウィンドウ（VSCode等）をElectronアプリでキャプチャし配信画面に表示
...
- **Go Live** - Electron経由でTwitch配信開始（コメント読み上げ・Git監視も自動開始）
- **配信停止** - 配信を停止
...
### Electronアプリ（Windows側配信+キャプチャ）

```bash
cd win-capture-app
npm install
npm run build
# → dist/win-unpacked/win-capture-app.exe が生成される
```

Web UIの「配信」タブ → 「ウィンドウキャプチャ」カードから「サーバー起動」でElectronアプリが自動起動される。詳細は [docs/window-capture.md](docs/window-capture.md) を参照。
