...
- [ ] Step 2: カテゴリプロンプトDB保存化（`prompt_content`カラム追加、`prompt_file`廃止）
- [ ] Step 3: 評価関数追加（`evaluate_lesson_quality()`, `evaluate_category_fit()`, `determine_targets()`）
- [ ] Step 4: `/improve` エンドポイント修正（target_sections空→3軸自動判定）
