...
- [ ] 授業中のテキストパネルのサイズ・文字サイズ改善 → [plans/lesson-text-panel-sizing.md](plans/lesson-text-panel-sizing.md)
- [ ] 「画面を見てね」とか「画面を読むね」という前置きは不自然なので無くす。どのようなプロンプトで回避可能か提案して欲しい。
- [ ] 授業の流れパネルは現在のセクションが明るく表示されるけど動作していない
