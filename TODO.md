...
- [ ] TODO表示が消える問題を修正
        ...
        検証手順:
        1. `curl http://localhost:$WEB_PORT/api/todo` → itemsが返るか
        2. `curl http://localhost:$WEB_PORT/overlay` → HTMLにtodo-panelが含まれるか
        3. OBS接続後: `curl http://localhost:$WEB_PORT/api/obs/diag` → overlay_sourceのURLとreachableを確認
