...
## ステータス: Phase 1-3 実装済み + 診断ログ強化済み

## 2026-03-21 配信検証結果

### 検証環境
- エンコーダ: h264_nvenc（GPU）
- ビットレート: 3500k / bufsize: 7000k
- 解像度: 1280x720 @ 30fps

### 実測データ（Session 3: 13:09～約45分間）
| メトリクス | 値 | 評価 |
|-----------|-----|------|
| FFmpeg speed | 1.03x → 0.87x（徐々に低下） | ✕ 要改善 |
| 実効fps | 30 → 26-27 | ✕ |
| フレームドロップ | 11,602 / 70,500 (16.4%) | ✕ |
| 音声ドロップ | 8,037+ | ✕ |
| パイプ書き込み遅延 | 最大550ms（100ms超が多数） | ✕ |
| NV12変換時間 | 1ms（問題なし） | ✅ |

### 原因特定
**ネットワークアップロード帯域不足がボトルネック**。h264_nvencエンコード自体は高速（1ms）だが、RTMP出力がリアルタイムに追いつけず、FFmpegの内部バッファが徐々に溜まり、パイプへの書き込みがブロックされる。speedが時間経過とともに線形に低下するパターンがこれを裏付ける。

### 改善対応
1. **診断ログ強化**（実装済み）:
   - 60秒ごとの配信サマリー（speed/fps/ドロップ率/パイプ遅延を一覧表示）
   - パイプ書き込み100ms超で個別警告
   - 段階的speed警告（0.95→0.90→0.85→0.80、推奨ビットレート表示）
   - 配信終了時の総合レポート
2. **次回配信の推奨設定**:
   - ビットレートを2500kに下げて検証（speed >= 1.0x を維持できるか）
   - `--bitrate 2500k` で起動
   - ネットワーク帯域テスト: `speedtest --simple` でアップロード速度確認
