...
`_build_structure_prompt()` に `main_content` パラメータを追加し、種別ルール＋事前分析コンテンツを**システムプロンプトに統合**する。
（ユーザープロンプト側は `extracted_text` がそのまま渡されるため、システムプロンプトで完結させる）

`generate_lesson_script_v2()` にも `main_content` パラメータを追加。
teacher.py の呼び出し元で DB の `lesson.main_content` を JSON パースして渡す。
