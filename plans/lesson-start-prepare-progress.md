...
当初はTTS生成中の進捗を管理画面に表示する案だったが、議論の結果「そもそもTTSが揃っていない状態で開始ボタンを押させない」方が素直と判断し、方針転換。

**実装**: `static/js/admin/teacher.js` の STEP 4 レンダリングで `ttsCacheMap` から全セクションキャッシュ済みかを判定。未生成セクションがあれば開始ボタンを灰色・disabled にし、ラベルを「TTS事前生成が必要 (N/M)」に変更、下部に赤字で「先に『TTS事前生成』ボタンで音声を生成してください」を表示。

ユーザーは既存の「TTS事前生成」ボタンでキャッシュを生成してから授業開始できる。開始を押した時点では音声準備済みなので、即再生が始まる。
