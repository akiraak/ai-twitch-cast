...
### Step 3.5: UIレイアウト改善（サイドバー化 + スクロール委譲） ✅ 完了

現在のレイアウト（縦積み2カード）をサイドバー+メインエリアに変更する。

#### 変更前
```
┌─────────────────────────────────┐
│ [plans] [docs]                  │  ← カード1（ディレクトリ切替＋ファイル一覧）
│ [file1] [file2] [file3] ...     │     ボタン横並びwrap
└─────────────────────────────────┘
┌─────────────────────────────────┐
│ filename.md              300文字 │  ← カード2（Markdown表示）
...
```
┌──────────────┬──────────────────┐
│ [plans][docs]│ filename.md 300字│
│              │                  │
│ file1.md     │ # Heading        │
│ file2.md  ◀  │ content...       │  ← ブラウザスクロールに委譲
│ file3.md     │ ...              │     max-height/overflow-y なし
│ ...          │                  │
└──────────────┴──────────────────┘
```

#### HTML変更 (`static/index.html`)

`tab-docs` の中を flexbox の横並びレイアウトに変更:

```html
<div class="tab-content" id="tab-docs">
  <div style="display:flex; gap:16px; align-items:flex-start;">
    <!-- サイドバー: ディレクトリ切替 + ファイル一覧（縦並び） -->
    <div class="card" style="min-width:200px; max-width:260px; flex-shrink:0; position:sticky; top:8px;">
      <h2>ドキュメント</h2>
      <div style="display:flex; gap:6px; margin-bottom:10px;">
        <button class="db-tab active" id="docs-dir-plans" onclick="switchDocsDir('plans')">plans</button>
        <button class="db-tab" id="docs-dir-docs" onclick="switchDocsDir('docs')">docs</button>
      ...
      <!-- ファイルボタンを縦並びに変更 -->
      <div id="docs-file-list" style="display:flex; flex-direction:column; gap:4px;"></div>
    </div>
    ...
    <div class="card" style="flex:1; min-width:0;">
      ...
      <!-- max-height/overflow-y を削除、ブラウザスクロールに委譲 -->
      <div id="docs-content" class="doc-modal-body">
        <p style="color:#9a88b5;">← ファイルを選択すると内容が表示されます</p>
      </div>
    </div>
...
**要点**:
- 外側を `display:flex; gap:16px` で横並び
- サイドバー: `min-width:200px; max-width:260px; flex-shrink:0` で固定幅、`position:sticky; top:8px` でスクロール時も追従
- ファイル一覧: `flex-direction:column` で縦並び（長いファイル名が読みやすい）
- メインエリア: `flex:1; min-width:0` で残りスペースを使用
- Markdown表示: `max-height`/`overflow-y`/`border`/`padding` のインラインスタイルを削除、ブラウザのスクロールに委譲

#### CSS変更

追加CSS不要（インラインスタイルの変更のみ）。

#### JS変更

なし（HTMLの構造変更のみ、docs.jsのDOM操作は同じID/クラスを使うので影響なし）。

### Step 4: テスト (`tests/test_api_docs_viewer.py` 新規) ✅ 完了
