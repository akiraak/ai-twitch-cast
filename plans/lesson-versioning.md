...
- `scripts/routes/teacher.py`:
  ...
  - `import-sections` のバージョン自動作成ロジック
  - `POST /api/lessons/{id}/start` の `version` パラメータ必須化
- テスト: `tests/test_api_teacher.py` に追加

### Step 3: 検証 & 部分改善API
- `POST /api/lessons/{id}/verify`: 元教材との整合性チェック（coverage/contradictions）
- `POST /api/lessons/{id}/improve`: `source_version` で改善元を指定、`target_sections` で対象セクションのみ再生成→新バージョン作成
- 改善プロンプトにカテゴリ別学習結果（`prompts/learnings/{category}.md` + `_common.md`）を自動注入
- プロンプト構築: `prompts/` に検証用・改善用プロンプトを追加
- テスト追加

### Step 4: 授業横断の学習ループAPI
- `POST /api/lessons/analyze-learnings?category=xxx`: カテゴリ別に注釈を収集→AIがパターン抽出→`prompts/learnings/{category}.md` に書き出し
- `GET /api/lessons/learnings`: カテゴリ別の学習結果・注釈統計を返す
- `POST /api/lessons/improve-prompt`: 学習結果をもとにプロンプト改善案をdiff生成（管理画面でユーザー承認後に適用）
- カテゴリ専用プロンプト作成機能（ベースプロンプト + description → AI生成）
- テスト追加

### Step 5: 授業再生エンジン対応
- `src/lesson_runner.py`: `version_number` パラメータ対応、TTSキャッシュパスの変更
- 旧キャッシュとの互換処理

### Step 6: UI実装
- `static/js/admin/teacher.js`:
  - カテゴリ選択UI（授業作成・編集時）
  - バージョン管理UI（Step 3 内: バージョン切替・再生・改善起点選択）
  - セクション注釈UI（◎/△/✕ ボタン + コメント欄）
  - 整合性チェック実行ボタン + 結果表示（抜け・矛盾の一覧）
  - 改善元バージョン選択 + 対象セクション選択 + 追加指示入力 + 部分再生成ボタン
  - バージョン差分比較UI（2バージョン並列、変更セクションのみ展開）
  - 学習ダッシュボード（カテゴリ別タブ・注釈統計・分析実行・学習結果閲覧/編集）
  - プロンプト改善UI（diff確認→承認/却下）
  - プロンプト・AI出力の全文表示（折りたたみ）
