...
ステータス: 完了
作成日: 2026-05-06
更新日: 2026-05-06（Step 6 完了: CLAUDE.md に「授業再生の「間」」セクション追加 / [TIMING] ログは INFO のまま保持してチューニング・トラブルシュートで活用）
...
### Step 6: ドキュメント更新（完了 / 2026-05-06）
1. `CLAUDE.md` に「## 授業再生の「間」」セクション追加（`scenes.json` の `lesson_timings` で一元管理 / ホットリロード可能 / 授業 JSON の `wait_seconds` は読み捨て）
2. `[TIMING]` ログ: ユーザー判断により **INFO レベルで保持**（チューニング・トラブルシュート・回帰検証で活用するため、削除/降格はしない）
3. `DONE.md` 追記・`TODO.md` 該当行削除・このプランの `ステータス: 完了` 化
