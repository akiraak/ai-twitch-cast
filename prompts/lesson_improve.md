...
- `display_properties` は **全セクションで必須**（省略しない）。コンテンツ量に応じて `maxHeight` / `width` / `fontSize` を必ずセットで指定する
  - 短い（1-2行）: `maxHeight 20-28 / width 35-45 / fontSize 1.8-2.2`
  - 中程度（3-5行）: `maxHeight 32-45 / width 45-55 / fontSize 1.6-1.8`
  - 長い（コード/リスト）: `maxHeight 50-65 / width 55-70 / fontSize 1.4-1.6`
  - 視聴者は1080p画面で見るためフォントは小さくしすぎない。内容に対してパネルが大きすぎないようにする
- 注釈にパネルサイズへの言及がある場合（「パネルが大きすぎる」「文字が小さい」等）、`display_properties` の値を調整する
