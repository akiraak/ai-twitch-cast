...
- **各セクションの最初のdialogue（speaker: teacher）で、`display_text` の内容を省略せず全文読み上げること**。視聴者は画面と音声の両方で内容を理解するため、先生が画面に表示されている内容をすべて声に出して読む。導入の一言（「画面を見てね」「読み上げるよ」など）を添えてから全文を読み上げ、その後に解説や対話に入る。`display_text` が長い場合（教材本文など）でも省略しない
