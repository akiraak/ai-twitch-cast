...
このワークフローでは、教師モードの授業コンテンツ（画像教材）を読み取り、対話形式の授業スクリプトを生成してDBにインポートする。Gemini APIによる自動生成とは独立した「Claude Code生成」として保存され、同じ教材でGemini版とClaude版を比較できる。
...
- 既存のGemini生成セクションがあっても上書きしない（generatorが別なので共存する）
