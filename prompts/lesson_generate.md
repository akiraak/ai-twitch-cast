...
| `display_properties` | object | No | パネル表示設定。`display_text` のコンテンツ量に応じてサイズを調整する |

### section_type の使い分け

| タイプ | 用途 |
|--------|------|
| `introduction` | 授業の導入。トピック紹介、興味を引く |
| `explanation` | メインの解説。教材内容を教える |
| `example` | 具体例、実演、応用 |
| `question` | 視聴者への問いかけ。`question`/`answer` フィールドを使う |
| `summary` | まとめ・振り返り |

> **間（wait）の長さは授業 JSON では指定しない**。dialogue 間 / セクション間 / question の解答前 wait はすべて `scenes.json` の `lesson_timings` で一元管理される。授業 JSON に `wait_seconds` を入れても無視されるので生成しないこと。
...
`display_text` のコンテンツ量に応じてパネルの表示サイズを調整する。**全セクションで必須**（省略しない）。

| プロパティ | 単位 | 範囲 | 説明 |
|-----------|------|------|------|
| `maxHeight` | % | 10-90 | パネル最大高さ。コンテンツ量に応じて指定 |
| `width` | % | 10-95 | パネル幅 |
| `fontSize` | vw | 0.5-3.0 | フォントサイズ |

**コンテンツ量に応じたガイドライン:**

| コンテンツ量 | maxHeight | width | fontSize |
|-------------|-----------|-------|----------|
| 短い（1-2行、タイトルのみ） | 20-28 | 35-45 | 1.8-2.2 |
| 中程度（3-5行） | 32-45 | 45-55 | 1.6-1.8 |
| 長い（コードブロック、リスト等） | 50-65 | 55-70 | 1.4-1.6 |

**方針**: 視聴者は1080p画面で見るので、**フォントは小さくしすぎない**。**パネルは内容に対して過剰に大きくしない**（短い内容に大きいパネルは余白が目立つ）。コンテンツ量に合わせて3つの値（maxHeight / width / fontSize）を必ずセットで指定する。
...
```json
{
  "sections": [
    ...
    },
    {
      ...
      "display_properties": {"maxHeight": 35, "fontSize": 1.4}
    },
    {
