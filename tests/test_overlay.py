...
def _patch_todo_project():
    """_get_todo_activeを"project"に固定（DBソースの影響を排除）"""
    return patch("scripts.routes.overlay._get_todo_active", return_value="project")
...
class TestGetTodo:
    ...
    async def test_missing_file_returns_empty(self, tmp_path):
        with _patch_todo_project(), \
             patch("scripts.routes.overlay.TODO_PATH", tmp_path / "nonexistent.md"):
    ...
    async def test_parse_todo_items(self, tmp_path):
        ...
        with _patch_todo_project(), \
             patch("scripts.routes.overlay.TODO_PATH", todo_file):
            from scripts.routes.overlay import get_todo
            result = await get_todo()
