...
def _patch_todo_project():
    """get_active_sourceを"project"に固定（DBソースの影響を排除）"""
    return patch("scripts.services.todo_service.get_active_source", return_value="project")
...
class TestGetTodo:
    ...
    async def test_missing_file_returns_empty(self, tmp_path):
        with _patch_todo_project(), \
             patch("scripts.services.todo_service.TODO_PATH", tmp_path / "nonexistent.md"):
    ...
    async def test_parse_todo_items(self, tmp_path):
        ...
        with _patch_todo_project(), \
             patch("scripts.services.todo_service.TODO_PATH", todo_file):
            from scripts.routes.overlay import get_todo
            result = await get_todo()
...
class TestCommonDefaults:
    ...
    def test_all_visual_items_have_common_properties(self):
        ...
        visual_items = ["avatar1", "avatar2", "subtitle", "todo"]
...
class TestOverlaySettingsAPI:
    ...
    def test_get_returns_common_properties(self, api_client):
        ...
        for item in ["avatar1", "avatar2", "subtitle", "todo"]:
    ...
    def test_default_values_when_no_db(self, api_client):
        ...
        data = resp.json()
        # 共通プロパティが型としてちゃんと返る
        assert isinstance(data["avatar1"]["positionX"], (int, float))
        assert isinstance(data["avatar1"]["bgOpacity"], (int, float))
